...
export default function Navbar() {
  ...
  useEffect(() => {
    const fn = () => {
      ...
      if (story) {
        setPastStory(window.scrollY + 72 >= story.offsetTop);
      }
              ...
              {LINKS.map(l => (
                <li key={l.href}>
                  <a href={l.href} target={l.external?"_blank":undefined} rel={l.external?"noopener noreferrer":undefined}
                    ...
                    style={{ color: pastStory ? "var(--c-text-1)" : "rgba(240,232,218,0.65)" }}>
                    {l.label}
            ...
            <button className="md:hidden flex flex-col gap-1.5 p-1 ml-auto z-10" onClick={() => setOpen(v => !v)} aria-label="Menú">
              {[0,1,2].map(i => (
                <motion.span key={i}
                  ...
                  style={{ background: pastStory ? "var(--c-text-1)" : "rgba(240,232,218,0.85)" }}
                />
              ))}
            </button>
          </motion.nav>
